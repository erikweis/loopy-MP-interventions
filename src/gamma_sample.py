...
def _prob_i_infected_given_gamma(state, i, t, infection_prob, sample, temporal = False):
    """Calculate the probability that node i is infected at time t,
    ...
    """    
    if temporal:
        return 1 - np.prod([(1 - state[k][i][max(t-d,0)]) for k, d in \
